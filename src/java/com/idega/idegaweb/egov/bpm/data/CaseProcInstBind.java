...
@NamedQueries({
	...
	@NamedQuery(name=CaseProcInstBind.getUniqueIds_BY_CASES_IDS_QUERY_NAME, query="select cp.uuid from CaseProcInstBind cp where cp.caseId in (:"+CaseProcInstBind.casesIdsParam + ")"),
	...
	@NamedQuery(
			name = CaseProcInstBind.QUERY_FIND_CASES_IDS_BY_UUIDS,
			query = "select cp.caseId from CaseProcInstBind cp where cp.uuid in (:" + CaseProcInstBind.uuidProp + ")"
	),
	@NamedQuery(
...
public class CaseProcInstBind implements Serializable {
	...
	public static final String BIND_BY_CASES_IDS_QUERY_NAME = "CaseProcInstBind.bindByCasesIdsQuery",
								...
								getUniqueIds_BY_CASES_IDS_QUERY_NAME = "CaseProcInstBind.getUniqueIdsByCasesIdsQuery",
