...
public class CasesSearchResultsHolderImpl implements CasesSearchResultsHolder {
		...
		for (int i = 0; i < values.size(); i++) {
			...
			if (list) {
				...
				for (int j = 0; j < varValues.size(); j++) {
					...
					SXSSFRow tmpRow = rows.get(rowIndex);
					if (tmpRow == null) {
	...
	private byte[] getExportedData(
		...
		ServletOutputStream out = response == null ?
				null :
				response.getOutputStream();

		FastByteArrayOutputStream streamOut = out == null ?
		...
		for (String processName: casesByProcessDefinition.keySet()) {
			...
			SXSSFSheet sheet = createdSheets.containsKey(sheetName) ?
		...
		try {
			if (out == null) {
				...
				response.setContentType(StringUtil.isEmpty(mimeType) ? "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" : mimeType);
	...
	public boolean doExportCases(String id, boolean exportContacts, boolean showCompany, boolean addDefaultFields, String category, HttpServletRequest request, HttpServletResponse response, String fileName) {
		...
		try {
			if (response == null) {
				LOGGER.warning("Response is not provided");
				return false;
			}

			cases = getAllCases(id);
			if (ListUtil.isEmpty(cases)) {
				LOGGER.warning("No cases found by ID: " + id);
				return false;
			}
